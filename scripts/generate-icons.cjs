...
function generatePNGIcon(size, filename) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (Math.abs(d - rad) < size * 0.018) {
        r = 30; g = 58; b = 43; // #1E3A2B Forest Green
      } else if (d < size * 0.38) {
        r = 252; g = 250; b = 245;
      }
      ...
      if (dMoon < moonR && dMoonCut > moonR * 0.78) {
        r = 212; g = 175; b = 55; // #D4AF37
      }

      // Tree Trunk & Roots
      const isTrunk = Math.abs(x - cx) < size * 0.03 && y >= cy - size * 0.05 && y <= cy + size * 0.18;
      if (isTrunk) {
        r = 61; g = 46; b = 30;
      }

      const dy = y - (cy + size * 0.18);
      if (dy > 0 && dy < size * 0.24) {
        const spread = dy * 0.85;
        if (Math.abs(Math.abs(x - cx) - spread) < size * 0.022) {
          r = 30; g = 58; b = 43;
        } else if (Math.abs(Math.abs(x - cx) - spread * 0.5) < size * 0.02) {
          r = 61; g = 46; b = 30;
        }
      }
